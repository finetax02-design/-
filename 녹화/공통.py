...
def _볼거리한줄(i, r, 무엇, 까닭):
    return {
        "몇번째": i + 1,
        "일자": r.get("s_date"),
        "거래처": r.get("nm_trade"),
        "사업자번호": r.get("no_bisocial"),
        "품명": r.get("nm_good"),
        "공급가액": r.get("mn_mnam"),
        "유형": r.get("ty_mth2"),
        "사유": r.get("cd_notdedct"),
        "전표상태": r.get("ty_jungstat"),
        "무엇": 무엇,
        "까닭": 까닭,
    }
...
def 검토거리(rows, 규칙):
    """사람이 눈으로 봐야 할 줄만 골라낸다.

    1,000곳을 다 훑을 수는 없다. 이것만 보면 되도록 추린다.
      규칙과 어긋난 불공 줄        이미 불공인데 사유가 규칙과 다르다
      전송된 뒤 규칙이 불공인 줄    이미 전송되어 손댈 수 없다
      규칙에 없는 거래처의 불공 줄  새로 생긴 것일 수 있다
    """
    볼것 = []
    for i, r in enumerate(rows):
        ...
        바람 = 규칙.get(biz)
        if ty == 불공 and 바람 and cd != 바람:
            볼것.append(_볼거리한줄(i, r, "불공사유",
                                    f"불공인데 사유가 {cd or '없음'} 입니다 (규칙은 {바람})"))
        elif ty == 불공 and not 바람 and cd not in 사유이름:
            볼것.append(_볼거리한줄(i, r, "불공사유",
                                    f"불공인데 사유가 {cd or '없음'} 입니다 (규칙에 없는 거래처)"))
        elif ty == 과세 and 바람 and 상태 == 전표확정:
            볼것.append(_볼거리한줄(i, r, "불공전환",
                                    f"규칙은 불공({바람})인데 이미 전송되어 못 바꿉니다"))
    return 볼것
...
def 계정과목채우기(page, 말하기):
    ...
    rows = 전표읽기(page)
    if not rows:
        return {"결과": "전표 없음", "성공": 0, "실패": 0, "건너뜀": 0, "볼거리": []}
    ...
    볼거리 = [_볼거리한줄(i, rows[i], "계정과목", why) for i, r, why in 건너뜀]
    for i, r, why in 건너뜀[:8]:
        말하기(f"      건너뜀 {i + 1}번째 {r.get('nm_trade')} : {why}")
    if not 할일:
        return {"결과": "채울 것 없음", "성공": 0, "실패": 0,
                "건너뜀": len(건너뜀), "볼거리": 볼거리}
    ...
    for x, why in 실패:
        볼거리.append(_볼거리한줄(x["row"], x["rec"], "계정과목",
                                  f"{x['side']} 넣기 실패: {why}"))
    return {"결과": "정상" if not 실패 else "도중 멈춤",
            "성공": 성공, "실패": len(실패), "건너뜀": len(건너뜀), "볼거리": 볼거리}
...
def 불공전환(page, 규칙, 말하기):
    ...
    for 사유 in sorted(사유이름):
        ...
        if not 본보기:
            ...
            for i in 대상:
                결과["볼거리"].append(_볼거리한줄(
                    i, rows[i], "불공전환",
                    f"사유 {사유} 로 바꿔야 하는데 본보기가 될 불공 줄이 화면에 없습니다"))
            continue
...
def 매입조회(page, 말하기, 기간무엇=None):
    """기간을 정하고 구분을 2.매입 으로 바꾼 뒤 조회를 누른다.

    기간무엇이 없으면 화면에 뜬 기본값 그대로 조회한다.
    연도는 고객사마다 다르므로 화면 위쪽 회계기간에서 얻는다.
    ...
    if 기간무엇:
        ...
        시작, 끝 = 기간계산(연도, 기간무엇)
        if not 시작:
            return -1, f"모르는 기간 [{기간무엇}]"
        됐나, 까닭 = 기간설정(page, 시작, 끝, 말하기)
        if not 됐나:
            return -1, f"기간을 못 정함: {까닭}"
