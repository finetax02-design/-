...
REVIEW = HERE / f"검토목록_{datetime.datetime.now():%Y%m%d_%H%M}.csv"
...
def 검토쓰기():
    """사람이 눈으로 봐야 할 줄만 한 파일에 모은다.

    1,000곳을 다 훑을 수는 없다. 이것만 보면 되도록 추린 것이다.
    """
    if not 볼거리:
        return
    머리 = ["고객사명", "몇번째", "일자", "거래처", "사업자번호", "품명",
            "공급가액", "유형", "사유", "전표상태", "무엇", "까닭"]
        ...
        with sync_playwright() as p:
            ...
            if 전표화면 is None:
                ...
                if 건수 < 0:
                    ...
                    for v in ㄱ.get("볼거리", []):
                        볼거리.append({"고객사명": 표적["고객사명"], **v})

                    지금줄 = 공통.전표읽기(전표화면) or []
                    규칙, 자세히 = 공통.불공규칙만들기(지금줄)
                    ...
                    마친줄 = 공통.전표읽기(전표화면) or 지금줄
                    남은것 = 공통.검토거리(마친줄, 규칙)
                    if 남은것:
                        say(f"    눈으로 봐야 할 줄 {len(남은것)}건")
                    for v in 남은것:
                        볼거리.append({"고객사명": 표적["고객사명"], **v})
...
say("")
if 볼거리:
    say(f"  눈으로 봐야 할 줄 모두 {len(볼거리)}건")
    say(f"  {REVIEW.name} 에 모아 두었습니다. 이것만 보시면 됩니다.")
else:
    say("  눈으로 봐야 할 줄은 없었습니다.")
say("")
